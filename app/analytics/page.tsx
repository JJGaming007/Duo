...
export default function AnalyticsPage() {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            if (!currentId) return
            try {
                const res = await fetch(`/api/analytics?userId=${currentId}`)
                if (res.ok) {
    ...
    const diff = stats.userCount - stats.partnerCount;
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <section className="space-y-4">
                ...
                <Card className="bg-zinc-900/20 border-zinc-800">
                    <CardContent className="h-64 flex items-end justify-between p-6 gap-2">
                        {(stats.recentProgress.length > 0 ? stats.recentProgress : Array(7).fill({ day: 0, time: 20 })).map((item, i) => (
            ...
            <Card className="bg-gradient-to-br from-zinc-900 to-zinc-950 border-zinc-800 overflow-hidden relative shadow-2xl">
                ...
                <CardHeader>
                    ...
                    <CardDescription>See how you stack up against your partner ({currentId === 'user-1' ? 'Partner A' : 'Partner B'} vs {currentId === 'user-1' ? 'Partner B' : 'Partner A'}).</CardDescription>
                </CardHeader>
                <CardContent>
